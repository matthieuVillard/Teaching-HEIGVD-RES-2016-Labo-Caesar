	...
	private class ReceptionistWorker implements Runnable {
		...
		private class ServantWorker implements Runnable {
			...
			public void run() {
				...
				try {
					LOG.info("Reading until client closes the connection...");

					while ((line = in.readLine()) != null) {
						if(line.toUpperCase().equals("KEY")){
							Random r = new Random();
							key = 1 + r.nextInt(25);
							LOG.info("Server generate " + key + " for client");
							...
							line = cesar.decrypt(line);
							LOG.info("Server received " + line + " from client");
							out.println(cesar.encrypt(line.toUpperCase()));
							out.flush();
						}
					}
